...
\maketitle
\CJKfamily{tzhrm}\makexeCJKactive
...
\myslide{Sketch Engine}
\MyLogo{\url{https://www.sketchengine.eu/}}
\begin{itemize}
\item corpus manager and text analysis software
\item it provides (among other things) word sketches:
  \\ one-page, automatic, corpus-derived summaries of a word's grammatical and collocational behaviour.
\item it supports and provides corpora in 90+ languages
\end{itemize}

\begin{itemize}
\item \cite{Kilgarriff:Rychlý:Smrž:Tugwell:2004}
\end{itemize}
  
\myslide{Google N-grams}
\MyLogo{\url{https://books.google.com/ngrams/}}

\begin{itemize}
\item  an online search engine that charts the frequencies of any set of search strings
  \\ using a yearly count of n-grams found in printed sources published between 1500 and 2019
  \\ in Google's text corpora in English, Chinese (simplified), French, German, Hebrew, Italian, Russian, or Spanish
\item texts are automatically POS tagged
\item you can search with limited wildcards
\item text comes from OCRs, has many errors in text and meta-data
\end{itemize}

\begin{itemize}
\item \cite{lin-etal-2012-syntactic}
\item \cite{Younes:Reips:2019}
  \\ \href{https://doi.org/10.1371/journal.pone.0213554}{10.1371/journal.pone.0213554}
\end{itemize}
    
\myslide{Universal Dependencies (UD)}
\MyLogo{\url{https://universaldependencies.org/}}
\begin{itemize}
\item is a framework for consistent annotation of grammar \\ (parts of speech, morphological features, and syntactic dependencies)
\item  across different human languages
\item an open community effort with over 200 treebanks in over 100 languages
\item many useful tools and interfaces
  \begin{itemize}
  \item \href{https://match.grew.fr/}{Grew Match} for matching trees
  \end{itemize}
\end{itemize}

\myslide{References}
\begin{itemize}
\item \cite{mcdonald-EtAl:2013:Short}
\item \cite{petrov-etal-2012-universal}
\item \cite{guibon-etal-2020-collaborative}
\end{itemize}
